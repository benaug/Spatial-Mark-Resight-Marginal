sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    lam.mnoID.nodes <- model$expandNodeNames(paste("lam.mnoID[",g,",1:",J.sight,",1:",K.sight,"]"))
    lam.um.nodes <- model$expandNodeNames(paste("lam.um[",g,",1:",J.sight,",1:",K.sight,"]"))
    lam.unk.nodes <- model$expandNodeNames(paste("lam.unk[",g,",1:",J.sight,",1:",K.sight,"]"))
    y.mark.nodes <- model$expandNodeNames(paste("y.mark[",g,",",i,",1:",J.mark,"]"))
    y.mID.nodes <- model$expandNodeNames(paste("y.mID[",g,",",i,",1:",J.sight,",1:",K.sight,"]"))
    y.mnoID.nodes <- model$expandNodeNames(paste("y.mnoID[",g,",1:",J.sight,",1:",K.sight,"]"))
    y.um.nodes <- model$expandNodeNames(paste("y.um[",g,",1:",J.sight,",1:",K.sight,"]"))
    y.unk.nodes <- model$expandNodeNames(paste("y.unk[",g,",1:",J.sight,",1:",K.sight,"]"))
    ...
    if(z==0){ #propose from prior
      ...
      if(inbox){
        ...
        if(i<=n.marked){ #if marked
          ...
          bigLam.marked.initial <- model$bigLam.marked[g,1:J.sight,1:K.sight]
          bigLam.premarked.initial <- model$bigLam.premarked[g,1:J.sight,1:K.sight] #need this one, too!
          #update proposed s
          ...
          bigLam.premarked.proposed <- bigLam.premarked.initial
          for(k in 1:K.sight){
            bigLam.marked.proposed[,k] <- bigLam.marked.proposed[,k] - model$lam[g,i,1:J.sight]*model$marked.status[g,i,k]
            bigLam.premarked.proposed[,k] <- bigLam.premarked.proposed[,k] - model$lam[g,i,1:J.sight]*(1-model$marked.status[g,i,k])
          }
          ...
          model$bigLam.marked[g,1:J.sight,1:K.sight] <<- bigLam.marked.proposed
          model$bigLam.premarked[g,1:J.sight,1:K.sight] <<- bigLam.premarked.proposed
