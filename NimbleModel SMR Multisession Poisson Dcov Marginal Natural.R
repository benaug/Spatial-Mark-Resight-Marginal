NimModel <- nimbleCode({
  ...
  for(g in 1:N.session){
    ...
    D0.UM[g] ~ dunif(0,100) #uninformative, diffuse dnorm on log scale can cause neg bias
    D0[g] <- D0.M[g] + D0.UM[g]
    ...
    lambda.N.M[g] <- D.intercept.M[g]*pi.denom[g] #Expected marked N
    lambda.N.UM[g] <- D.intercept.UM[g]*pi.denom[g] #Expected unmarked N
