...
zSampler <- nimbleFunction(
  ...
  run = function(){
    bigLam.marked.initial <- model$bigLam.marked[g,1:J]
    ...
    for(up in 1:z.ups[1]){ #how many updates per iteration?
      ...
      if(updown==0){ #subtract
        ...
        if(n.z.on>0){ #skip if no marked z's to turn off, otherwise nimble will crash
          ...
          if(pick<=n.marked){ #is this individual detected?, if so, reject
            reject <- TRUE
          }
          if(!reject){
            ...
            #turn off
            bigLam.marked.proposed <- bigLam.marked.initial - model$lam[g,pick,1:J] #subtract these out before calculate
            model$calculate(lam.nodes[pick])
            model$bigLam.marked[g,1:J] <<- bigLam.marked.proposed
            ...
            if(accept){
              ...
              mvSaved["lam",1][g,pick,] <<- model[["lam"]][g,pick,]
              mvSaved["bigLam.marked",1][g,1:J] <<- model[["bigLam.marked"]][g,1:J]
              mvSaved["lam.mnoID",1][g,1:J] <<- model[["lam.mnoID"]][g,1:J]
              mvSaved["lam.unk",1][g,1:J] <<- model[["lam.unk"]][g,1:J]
              ...
              model[["lam"]][g,pick,] <<- mvSaved["lam",1][g,pick,]
              model[["bigLam.marked"]][g,1:J] <<- mvSaved["bigLam.marked",1][g,1:J]
              model[["lam.mnoID"]][g,1:J] <<- mvSaved["lam.mnoID",1][g,1:J]
              model[["lam.unk"]][g,1:J] <<- mvSaved["lam.unk",1][g,1:J]
        ...
        if(model$N.M[g] < M1){ #cannot update if z maxed out. Need to raise M1
          ...
          #turn on
          model$calculate(lam.nodes[pick])
          bigLam.marked.proposed <- bigLam.marked.initial + model$lam[g,pick,1:J] #add these in after calculate
          model$bigLam.marked[g,1:J] <<- bigLam.marked.proposed
          model$calculate(lam.mnoID.nodes)
          model$calculate(lam.unk.nodes)
    ...
    bigLam.unmarked.initial <- model$bigLam.unmarked[g,1:J]
    for(up in 1:z.ups[2]){ #how many updates per iteration?
      ...
      if(updown==0){#subtract
        ...
        if(n.z.on>0){ #skip if no unmarked z's to turn off, otherwise nimble will crash
          ...
          if(model$N.UM[g]==1){ #is this the last unmarked individual?
            reject <- TRUE
          }
          if(!reject){
            #get initial logprobs for N and y
            ...
            #turn off
            bigLam.unmarked.proposed <- bigLam.unmarked.initial - model$lam[g,pick,1:J] #subtract these out before calculate
            model$calculate(lam.nodes[pick])
            model$bigLam.unmarked[g,1:J] <<- bigLam.unmarked.proposed
            ...
            if(accept){
              ...
              mvSaved["z",1][g,pick] <<- model[["z"]][g,pick]
              mvSaved["lam",1][g,pick,] <<- model[["lam"]][g,pick,]
              mvSaved["bigLam.unmarked",1][g,1:J] <<- model[["bigLam.unmarked"]][g,1:J]
              mvSaved["lam.um",1][g,1:J] <<- model[["lam.um"]][g,1:J]
              mvSaved["lam.unk",1][g,1:J] <<- model[["lam.unk"]][g,1:J]
              ...
              model[["z"]][g,pick] <<- mvSaved["z",1][g,pick]
              model[["lam"]][g,pick,] <<- mvSaved["lam",1][g,pick,]
              model[["bigLam.unmarked"]][g,1:J] <<- mvSaved["bigLam.unmarked",1][g,1:J]
              model[["lam.um"]][g,1:J] <<- mvSaved["lam.um",1][g,1:J]
              model[["lam.unk"]][g,1:J] <<- mvSaved["lam.unk",1][g,1:J]
        ...
        if(model$N.UM[g] < M.both-M1){ #cannot update if z maxed out. Need to raise M2
          ...
          #turn on
          model$calculate(lam.nodes[pick])
          bigLam.unmarked.proposed <- bigLam.unmarked.initial + model$lam[g,pick,1:J] #add these in after calculate
          model$bigLam.unmarked[g,1:J] <<- bigLam.unmarked.proposed
          model$calculate(lam.um.nodes)
          model$calculate(lam.unk.nodes)
