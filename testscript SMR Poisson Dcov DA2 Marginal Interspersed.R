...
#It handles "premarked" scenarios with interspersed marking and sighting.
...
library(nimble)
source("sim.SMR.Dcov.Interspersed.R")
source("NimbleModel SMR Poisson Dcov DA2 Marginal Interspersed.R")
source("NimbleFunctions SMR Poisson Dcov DA2 Marginal Interspersed.R")
source("init.SMR.Dcov.Interspersed.R")
source("sSampler Poisson Dcov Marginal Interspersed.R")
source("mask.check.R")
...
theta.marked <- c(0.75,.15,.1) #P(ID, Marked no ID, unk status). must sum to 1
...
data <- sim.SMR.Dcov.Interspersed(D.beta0=D.beta0,D.beta1=D.beta1,res=res,
...
str(data$marked.status) #history of which occasions marked individuals are marked
...
#make inits for theta.marked and theta.unmarked
#each must sum to 1
theta.marked.init <- c(0.5,0.25,0.25)
theta.unmarked.init <- c(0,0.75,0.25) #first cell for theta.unmarked.init should be 0

inits <- list(lam0=lam0,sigma=sigma,theta.marked=theta.marked.init,theta.unmarked=theta.unmarked.init)
...
# constants <- list(n.marked=n.marked,M=M,J=J,K=K,K2D=data$K2D,
...
constants <- list(n.marked=n.marked,M=M,J=J,K=data$K,K2D=data$K2D,
                  D.cov=data$D.cov,cellArea=data$cellArea,n.cells=data$n.cells,
                  xlim=nimbuild$xlim,ylim=nimbuild$ylim,res=data$res,
...
#                   type = 'sSampler',control=list(i=i,J=J,K=K,res=data$res,n.cells.x=data$n.cells.x,n.cells.y=data$n.cells.y,
