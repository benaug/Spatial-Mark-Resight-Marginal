NimModel <- nimbleCode({
  ...
  p0.p ~ dunif(0,1) #marking process, first capture p0
  p0.c ~ dunif(0,1) #marking process, subsequent capture p0
  ...
  for(i in 1:M){
    ...
    kern[i,1:J.mark] <- GetKern(s = s[i,1:2], X = X.mark[1:J.mark,1:2], J=J.mark, sigma=sigma, z=z[i])
    pd.p[i,1:J.mark] <- GetDetectionProb(kern = kern[i,1:J.mark], p0=p0.p, J=J.mark, z=z[i])
    pd.c[i,1:J.mark] <- GetDetectionProb(kern = kern[i,1:J.mark], p0=p0.c, J=J.mark, z=z[i])
    y.mark.p[i,1:J.mark] ~ dBinomialVector(pd=pd.p[i,1:J.mark],K1D=K1D.mark.p[i,1:J.mark],z=z[i])
    y.mark.c[i,1:J.mark] ~ dBinomialVector(pd=pd.c[i,1:J.mark],K1D=K1D.mark.c[i,1:J.mark],z=z[i])
  }
  ...
  for(i in 1:n.tel.inds){
    for(m in 1:n.locs.ind[i]){
      locs[i,m,1] ~ dnorm(s[tel.ID[i],1],sd=sigma)
      locs[i,m,2] ~ dnorm(s[tel.ID[i],2],sd=sigma)
    }
  }
})# end model