sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    y.mID.nodes <- model$expandNodeNames(paste("y.mID[",i,",1:",J,",1:",K,"]"))
    ...
    lam.mnoID.nodes <- model$expandNodeNames(paste("lam.mnoID[1:",J,",1:",K,"]"))
    lam.um.nodes <- model$expandNodeNames(paste("lam.um[1:",J,",1:",K,"]"))
    lam.unk.nodes <- model$expandNodeNames(paste("lam.unk[1:",J,",1:",K,"]"))
    ...
    if(z==0){#propose from unifrom prior
      ...
      if(inbox){
        ...
        if(i<=n.marked){ #if marked
          ...
          bigLam.premarked.initial <- model$bigLam.premarked #need this one, too!
          #update proposed s
          ...
          for(k in 1:K){
            bigLam.marked.proposed[,k] <- bigLam.marked.proposed[,k] - model$lam[i,]*model$marked.status[i,k]
            bigLam.premarked.proposed[,k] <- bigLam.premarked.proposed[,k] - model$lam[i,]*(1-model$marked.status[i,k])
          }
          ...
          model$bigLam.premarked <<- bigLam.premarked.proposed
