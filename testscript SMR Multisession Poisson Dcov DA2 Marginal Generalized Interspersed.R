#multisession version of Poisson Dcov Marginal Generalized Interspersed

library(nimble)
source("sim.SMR.multisession.Dcov.Generalized.Interspersed.R")
source("sim.SMR.Dcov.Generalized.Interspersed.R") #used by multisession simulator
source("NimbleModel SMR Multisession Poisson Dcov Marginal Generalized Interspersed.R")
source("NimbleFunctions SMR Multisession Poisson Dcov Marginal Generalized Interspersed.R")
source("init.SMR.multisession.Dcov.Generalized.Interspersed.R")
source("init.SMR.Dcov.Generalized.Interspersed.R") #used by multisession initializer
source("sSampler Multisession Poisson Dcov Marginal Generalized Interspersed.R")
source("mask.check.R")
...
K.mark <- c(5,5,5) #number of marking occasions
K.sight <- c(10,10,10) #number of sighting occasions
...
#For each session ,a character vector of length K.mark[g] + K.sight[g] specifying the order of the marking
#and sighting occasions. Vector elements are either "M" or "S" arranged in the order the marking
#and sighting occasions occurred. There must be K.mark M's and K.sight S's.
#Data simulator will check these requirements.

#doing different orderings of K.mark=5, K.sight=10
K.order <- vector("list",N.session)
K.order[[1]] <- c("S","M","S","M","S","M","S","M","S","M","S","S","S","S","S")
K.order[[2]] <- c("M","S","S","M","S","M","S","M","S","M","S","S","S","S","S")
K.order[[3]] <- c("M","S","M","S","M","S","M","S","M","S","S","S","S","S","S")
...
data <- sim.SMR.multisession.Dcov.Generalized.Interspersed(N.session=N.session,D.beta0=D.beta0,
                D.beta1=D.beta1,res=res,D.cov=D.cov,InSS=InSS,
                theta.marked=theta.marked,theta.unmarked=theta.unmarked,
                lam0=lam0,p0=p0,sigma=sigma,K.mark=K.mark,K.sight=K.sight,K.order=K.order,
...
str(data[[g]]$marked.status) #history of which sighting occasions marked individuals are marked
...
inits <- list(p0=rep(0.5,N.session),lam0=rep(1,N.session),sigma=rep(1,N.session), #initializing with 1 parameter per session, just set all to same value
              theta.marked=theta.marked.init,theta.unmarked=theta.unmarked.init)

#This function structures the simulated data to fit the model in Nimble (some more restructing below)
nimbuild <- init.SMR.multisession.Dcov.Generalized.Interspersed(data,inits,M=M)
...
for(g in 1:N.session){
  ...
  y.mark2D <- apply(data[[g]]$y.mark,c(1,2),sum)
...
#                   K.sight=nimbuild$K.sight,K1D.mark=nimbuild$K1D.mark,K2D.sight=nimbuild$K2D.sight,
...
#                 marked.status=nimbuild$marked.status, #individual by occasion marked status for marked guys
...
for(g in 1:N.session){
  ...
  y.um.nodes <- Rmodel$expandNodeNames(paste("y.um[",g,",1:",J.sight[g],",1:",K.sight[g],"]"))
  y.unk.nodes <- Rmodel$expandNodeNames(paste("y.unk[",g,",1:",J.sight[g],",1:",K.sight[g],"]"))
  bigLam.unmarked.nodes <- Rmodel$expandNodeNames(paste("bigLam.unmarked[",g,",1:",J.sight[g],"]")) #only need this in calcNodes
  lam.um.nodes <- Rmodel$expandNodeNames(paste("lam.um[",g,",1:",J.sight[g],",1:",K.sight[g],"]"))
  lam.unk.nodes <- Rmodel$expandNodeNames(paste("lam.unk[",g,",1:",J.sight[g],",1:",K.sight[g],"]"))
  ...
  conf$addSampler(target = paste("N"),
                  type = 'zSampler',control = list(g=g,z.ups=z.ups[g],
                                                   ...
                                                   K.sight=K.sight[g],n.marked=n.marked[g],M=M[g],
...
#                     type = 'sSampler',control=list(i=i,g=g,J.mark=J.mark[g],J.sight=J.sight[g],K.sight=K.sight[g],n.cells=nimbuild$n.cells[g],
...
for(g in 1:N.session){
  for(i in 1:M[g]){
    if(i %in% nimbuild$tel.inds[g,]){#inds with telemetry
      conf$addSampler(target = paste("s[",g,",",i,", 1:2]", sep=""),
                      type = 'sSampler',control=list(i=i,g=g,J.mark=J.mark[g],J.sight=J.sight[g],K.sight=K.sight[g],res=nimbuild$res[g],n.cells=nimbuild$n.cells[g],
                                                     n.cells.x=nimbuild$n.cells.x[g],n.cells.y=nimbuild$n.cells.y[g],
                                                     xlim=nimbuild$xlim[g,],ylim=nimbuild$ylim[g,],res=nimbuild$res[g],
...
#all sighting stuff off with s update
Cmcmc$run(5,reset=FALSE)
aa=Cmodel$bigLam.marked
bb=Cmodel$bigLam.premarked
cc=Cmodel$bigLam.unmarked
dd=Cmodel$logProb_y.mID
ee=Cmodel$logProb_y.mnoID
ff=Cmodel$logProb_y.um
gg=Cmodel$logProb_y.unk
hh=Cmodel$lam.um
ii=Cmodel$lam.unk
kk=Cmodel$logProb_y.mark
ll=Cmodel$lam
Cmodel$calculate()
range(aa-Cmodel$bigLam.marked)
range(bb-Cmodel$bigLam.premarked)
range(cc-Cmodel$bigLam.unmarked)
range(dd-Cmodel$logProb_y.mID,na.rm=TRUE)
range(ee-Cmodel$logProb_y.mnoID)
round(rowSums(ff-Cmodel$logProb_y.um),3) #z
round(rowSums(gg-Cmodel$logProb_y.unk),3) #z
round(rowSums(hh-Cmodel$lam.um),3) #z
round(rowSums(ii-Cmodel$lam.unk),3) #z
range(kk-Cmodel$logProb_y.mark)
range(ll-Cmodel$lam)
