      ...
      if(updown==0){ #subtract
        ...
        if(n.z.on>0){ #skip if no unmarked z's to turn off, otherwise nimble will crash
          ...
          if(!reject){
            ...
            #turn off
            bigLam.all.proposed <- bigLam.all.initial - model$lam[g,pick,1:J] #subtract these out before calculate
            ...
            if(accept){
              ...
              mvSaved["bigLam.all",1][g,1:J] <<- model[["bigLam.all"]][g,1:J]
              ...
              model[["bigLam.all"]][g,1:J] <<- mvSaved["bigLam.all",1][g,1:J]
        ...
        if(model$N[g] < M){  #cannot update if z maxed out. Need to raise M
          ...
          #turn on
          model$calculate(lam.nodes[pick])
          bigLam.all.proposed <- bigLam.all.initial + model$lam[g,pick,1:J] #add these in after calculate
          model$bigLam.all[g,1:J] <<- bigLam.all.proposed
