sSamplerMarked <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    s.nodes <- model$expandNodeNames(paste("s.m[",g,",",i,",",1:2,"]"))
    ...
    y.mID.nodes <- model$expandNodeNames(paste("y.mID[",g,",",i,",1:",J,"]"))
    y.mnoID.nodes <- model$expandNodeNames(paste("y.mnoID[",g,",1:",J,"]"))
    lam.nodes <- model$expandNodeNames(paste("lam.m[",g,",",i,",1:",J,"]"))
    lam.mnoID.nodes <- model$expandNodeNames(paste("lam.mnoID[",g,",1:",J,"]"))
  ...
  run = function(){
    s.cand <- c(rnorm(1,model$s.m[g,i,1],scale), rnorm(1,model$s.m[g,i,2],scale))
    ...
    if(inbox){
      ...
      bigLam.marked.proposed <- bigLam.marked.proposed + model$lam.m[g,i,1:J]
      ...
      lp_initial <- lp_initial_s + lp_initial_y.mID + lp_initial_y.mnoID
      lp_proposed <- lp_proposed_s + lp_proposed_y.mID + lp_proposed_y.mnoID
...
sSamplerAll <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    y.all.nodes <- model$expandNodeNames(paste("y.all[",g,",1:",J,"]"))
  ...
  run = function() {
    ...
    if(z==0){ #propose from prior
      ...
      if(inbox){
        ...
        lp_initial_s <- model$getLogProb(s.nodes)
        lp_initial_y.all <- model$getLogProb(y.all.nodes)
        #pull this out of model object
        bigLam.all.initial <- model$bigLam.all[g,1:J]
        #update proposed s
        model$s[g,i,1:2] <<- s.cand
        lp_proposed_s <- model$calculate(s.nodes) #proposed logprob for s.nodes
        #subtract these out before calculating lam
        bigLam.all.proposed <- bigLam.all.initial - model$lam[g,i,1:J]
        #make sure you didn't end up with any negative numbers due to machine precision
        bigLam.all.proposed[bigLam.all.proposed<0] <- 0
        model$calculate(lam.nodes) #update lam nodes
        #add these in after calculating lam
        bigLam.all.proposed <- bigLam.all.proposed + model$lam[g,i,1:J]
        #put bigLam in model object
        model$bigLam.all[g,1:J] <<- bigLam.all.proposed
        lp_proposed_y.all <- model$calculate(y.all.nodes)
        lp_initial <- lp_initial_s + lp_initial_y.all
        lp_proposed <- lp_proposed_s + lp_proposed_y.all
        log_MH_ratio <- lp_proposed - lp_initial
        accept <- decide(log_MH_ratio)
        if(accept) {
          copy(from = model, to = mvSaved, row = 1, nodes = calcNodes, logProb = TRUE)
