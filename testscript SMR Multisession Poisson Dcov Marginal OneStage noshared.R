...
source("NimbleModel SMR Multisession Poisson Dcov Marginal OneStage noshared.R")
...
Niminits <- list(N=N.init,z=nimbuild$z.init,s=nimbuild$s.init,D0=D0.init,D.beta1=rep(0,N.session),
                 lam0=rep(1,N.session),sigma=rep(1,N.session),theta.thin=rep(0.5,N.session))
...
parameters <- c('D0','D.beta1','lambda.N','N','lam0','sigma','theta.thin')
...
for(g in 1:N.session){
  conf$addSampler(target = c(paste("lam0[",g,"]"),paste("sigma[",g,"]")),type = 'RW_block',
