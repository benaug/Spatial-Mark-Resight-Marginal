...
#This version has a behavioral response to capture, typical Mb. But for model fitting, we
#use M x J first and subsequent capture histories instead of the full M x J x K capture history
#to speed up model fitting. But does not allow occasion effects.
...
library(nimble)
source("sim.SMR.Dcov.Generalized.Mb.R")
source("NimbleModel SMR Poisson Dcov DA2 Marginal Generalized Mb.R")
source("NimbleFunctions SMR Poisson Dcov DA2 Marginal Generalized Mb.R")
source("init.SMR.Dcov.Generalized.Mb.R")
source("sSampler Poisson Dcov Marginal Generalized Mb.R")
source("mask.check.R")
...
####Simulate some data####
p0.p <- 0.15 #marking process p0, first capture
p0.c <- 0.5 #marking process p0, subsequent capture
...
data <- sim.SMR.Dcov.Generalized.Mb(D.beta0=D.beta0,D.beta1=D.beta1,res=res,
                D.cov=D.cov,InSS=InSS,
                theta.marked=theta.marked,theta.unmarked=theta.unmarked,
                lam0=lam0,p0.p=p0.p,p0.c=p0.c,sigma=sigma,K.mark=K.mark,K.sight=K.sight,
...
str(data$y.mark) #marking process captures. 3D for Mb
...
inits <- list(p0.p=0.1,p0.c=0.1,lam0=1,sigma=1)
...
Niminits <- list(z=nimbuild$z,s=nimbuild$s,D0=D0.init,D.beta1=0,
                 ...
                 p0.p=inits$p0.p,p0.c=inits$p0.c,lam0=inits$lam0,sigma=inits$sigma)
...
#                   J.mark=J.mark,K1D.mark.p=nimbuild$K1D.mark.p,
#                   K1D.mark.c=nimbuild$K1D.mark.c,
...
# 
# Nimdata <- list(y.mark.p=nimbuild$y.mark.p, #marking process, first captures
#                 y.mark.c=nimbuild$y.mark.c, #marking process, subsequent captures
...
parameters <- c('D0','lambda.N','p0.p','p0.c','lam0','sigma','theta.marked','theta.unmarked',
                'N','D.beta1')
...
config.nodes <- c('p0.p','p0.c','lam0','sigma','theta.marked','theta.unmarked[2:3]')
# config.nodes <- c()
...
y.mark.p.nodes <- Rmodel$expandNodeNames("y.mark.p")
y.mark.c.nodes <- Rmodel$expandNodeNames("y.mark.c")
kern.nodes <- Rmodel$expandNodeNames("kern")
pd.p.nodes <- Rmodel$expandNodeNames("pd.p")
pd.c.nodes <- Rmodel$expandNodeNames("pd.c")
...
calcNodes <- c(N.node,kern.nodes,pd.p.nodes,pd.c.nodes,lam.nodes,bigLam.unmarked.nodes,
               lam.um.nodes,lam.unk.nodes,y.mark.p.nodes,y.mark.c.nodes,y.um.nodes,y.unk.nodes)
conf$addSampler(target = paste("N"),
                type = 'zSampler',control = list(z.ups=z.ups,J.sight=J.sight,
                                                 n.marked=n.marked,M=M,N.node=N.node,
                                                 pd.p.nodes=pd.p.nodes,pd.c.nodes=pd.c.nodes,
                                                 kern.nodes=kern.nodes,lam.nodes=lam.nodes,
                                                 lam.um.nodes=lam.um.nodes,lam.unk.nodes=lam.unk.nodes,
                                                 y.mark.p.nodes=y.mark.p.nodes,
                                                 y.mark.c.nodes=y.mark.c.nodes,
                                                 y.um.nodes=y.um.nodes,
...
Cmcmc$run(1250,reset=FALSE) #short run for demonstration. can keep running this line to get more samples
end.time <- Sys.time()
