...
zSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control){
    ...
    M <- control$M
    z.ups <- control$z.ups
    y.mark.p.nodes <- control$y.mark.p.nodes
    y.mark.c.nodes <- control$y.mark.c.nodes
    y.um.nodes <- control$y.um.nodes
    y.unk.nodes <- control$y.unk.nodes
    kern.nodes <- control$kern.nodes
    pd.p.nodes <- control$pd.p.nodes
    pd.c.nodes <- control$pd.c.nodes
      ...
      if(updown==0){#subtract
        ...
        if(n.z.on>0){ #skip if no unmarked z's to turn off, otherwise nimble will crash
          ...
          if(!reject){
            ...
            lp.initial.y.mark.p <- model$getLogProb(y.mark.p.nodes[pick])
            lp.initial.y.mark.c <- model$getLogProb(y.mark.c.nodes[pick])
            ...
            lp.proposed.y.mark.p <- model$calculate(y.mark.p.nodes[pick])
            lp.proposed.y.mark.c <- model$calculate(y.mark.c.nodes[pick])
            lp.proposed.y.um <- model$calculate(y.um.nodes)
            lp.proposed.y.unk <- model$calculate(y.unk.nodes)

            #MH step
            log_MH_ratio <- (lp.proposed.N + lp.proposed.y.mark.p + lp.proposed.y.mark.c + lp.proposed.y.um + lp.proposed.y.unk) -
              (lp.initial.N + lp.initial.y.mark.p + lp.initial.y.mark.c + lp.initial.y.um + lp.initial.y.unk)
            ...
            if(accept) {
              ...
              mvSaved["lam.um",1][1:J.sight] <<- model[["lam.um"]][1:J.sight]
              mvSaved["lam.unk",1][1:J.sight] <<- model[["lam.unk"]][1:J.sight]
              ...
              model[["lam.um"]][1:J.sight] <<- mvSaved["lam.um",1][1:J.sight]
              model[["lam.unk"]][1:J.sight] <<- mvSaved["lam.unk",1][1:J.sight]
              model$calculate(y.mark.p.nodes[pick])
              model$calculate(y.mark.c.nodes[pick])
