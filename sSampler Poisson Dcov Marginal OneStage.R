sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    y.um.nodes <- model$expandNodeNames("y.all")
    ...
    if(z==0){#propose from prior
      ...
      if(inbox){
        ...
        if(i<=n.marked){ #if marked
          ...
          lp_initial <- lp_initial_s + lp_initial_y.mID + lp_initial_y.mnoID + lp_initial_y.all
          lp_proposed <- lp_proposed_s + lp_proposed_y.mID + lp_proposed_y.mnoID + lp_proposed_y.all
        }else{ #else unmarked
          #update proposed s
          model$s[i, 1:2] <<- s.cand
          lp_proposed_s <- model$calculate(s.nodes) #proposed logprob for s.nodes
