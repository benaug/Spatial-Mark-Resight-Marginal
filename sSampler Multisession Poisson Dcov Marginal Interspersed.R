sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    y.mID.nodes <- model$expandNodeNames(paste("y.mID[",g,",",i,",1:",J,",1:",K,"]"))
    y.mnoID.nodes <- model$expandNodeNames(paste("y.mnoID[",g,",1:",J,",1:",K,"]"))
    y.um.nodes <- model$expandNodeNames(paste("y.um[",g,",1:",J,",1:",K,"]"))
    y.unk.nodes <- model$expandNodeNames(paste("y.unk[",g,",1:",J,",1:",K,"]"))
    lam.nodes <- model$expandNodeNames(paste("lam[",g,",",i,",1:",J,"]"))
    lam.mnoID.nodes <- model$expandNodeNames(paste("lam.mnoID[",g,",1:",J,",1:",K,"]"))
    lam.um.nodes <- model$expandNodeNames(paste("lam.um[",g,",1:",J,",1:",K,"]"))
    lam.unk.nodes <- model$expandNodeNames(paste("lam.unk[",g,",1:",J,",1:",K,"]"))
    ...
    if(z==0){#propose from prior
      ...
      if(inbox){
        ...
        if(i<=n.marked){ #if marked
          ...
          bigLam.marked.initial <- model$bigLam.marked[g,1:J,1:K]
          bigLam.premarked.initial <- model$bigLam.premarked[g,1:J,1:K] #need this one, too!
          #update proposed s
          ...
          model$bigLam.marked[g,1:J,1:K] <<- bigLam.marked.proposed
          model$bigLam.premarked[g,1:J,1:K] <<- bigLam.premarked.proposed
          ...
          lp_initial <- lp_initial_s + lp_initial_y.mID + lp_initial_y.mnoID + lp_initial_y.um + lp_initial_y.unk
          lp_proposed <- lp_proposed_s + lp_proposed_y.mID + lp_proposed_y.mnoID + lp_proposed_y.um + lp_proposed_y.unk
        }else{ #else unmarked
          lp_initial_y.um <- model$getLogProb(y.um.nodes)
          #pull this out of model object
