        ...
        if(n.z.on>0){ #skip if no unmarked z's to turn off, otherwise nimble will crash
          ...
          if(!reject){
            ...
            if(accept){
              ...
              mvSaved["lam.um",1][1:J,1:K] <<- model[["lam.um"]][1:J,1:K]
              mvSaved["lam.unk",1][1:J,1:K] <<- model[["lam.unk"]][1:J,1:K]
              ...
              model[["lam.um"]][1:J,1:K] <<- mvSaved["lam.um",1][1:J,1:K]
              model[["lam.unk"]][1:J,1:K] <<- mvSaved["lam.unk",1][1:J,1:K]
