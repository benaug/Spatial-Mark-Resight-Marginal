...
source("sSampler Poisson Dcov Marginal OneStage.R")
source("mask.check.R")
...
nimbuild <- init.SMR.Dcov.OneStage(data,inits,M=M)
...
y.all.nodes <- Rmodel$expandNodeNames("y.all")
bigLam.all.nodes <- Rmodel$expandNodeNames("bigLam.all") #only need this in calcNodes
