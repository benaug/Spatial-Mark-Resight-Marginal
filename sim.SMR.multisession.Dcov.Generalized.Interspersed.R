...
sim.SMR.multisession.Dcov.Generalized.Interspersed <-
  function(N.session=NA,D.beta0=NA,D.beta1=NA,D.cov=NA,InSS=NA,res=NA,
           ...
           theta.marked=NA,theta.unmarked=NA,K1D.mark=NA,K2D.sight=NA,
           K.order=NA,tlocs=NA,obstype="poisson"){
    ...
    if(length(K.order)!=N.session)stop("K.order must be of length N.session")
    ...
    if(any(is.na(K2D.sight))){
      ...
      K2D.sight <- vector("list",N.session)
      for(g in 1:N.session){
    ...
    for(g in 1:N.session){
      data[[g]] <- sim.SMR.Dcov.Generalized.Interspersed(D.beta0=D.beta0[g],D.beta1=D.beta1[g],res=res[g],D.cov=D.cov[[g]],InSS=InSS[[g]],
                                            ...
                                            K1D.mark=K1D.mark[[g]],K2D.sight=K2D.sight[[g]],
                                            X.mark=X.mark[[g]],X.sight=X.sight[[g]],xlim=xlim[g,],ylim=ylim[g,],tlocs=tlocs[g],
                                            obstype=obstype,K.order=K.order[[g]])
    }
    return(data)
  }