sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    lam.um.nodes <- model$expandNodeNames(paste("lam.um[",g,",1:",J,"]"))
    lam.unk.nodes <- model$expandNodeNames(paste("lam.unk[",g,",1:",J,"]"))
    y.mID.nodes <- model$expandNodeNames(paste("y.mID[",g,",",i,",1:",J,"]"))
    y.mnoID.nodes <- model$expandNodeNames(paste("y.mnoID[",g,",1:",J,"]"))
    y.um.nodes <- model$expandNodeNames(paste("y.um[",g,",1:",J,"]"))
    y.unk.nodes <- model$expandNodeNames(paste("y.unk[",g,",1:",J,"]"))
    ...
    if(z==0){ #propose from prior
      ...
      if(inbox){
        ...
        if(i<=M1){ #if marked
          ...
          bigLam.marked.proposed <- bigLam.marked.initial - model$lam[g,i,1:J]
          ...
          bigLam.marked.proposed <- bigLam.marked.proposed + model$lam[g,i,1:J]
