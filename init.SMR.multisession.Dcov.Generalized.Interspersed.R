...
init.SMR.multisession.Dcov.Generalized.Interspersed <- function(data,inits=NA,M=NA){
  ...
  for(g in 1:N.session){
    ...
    init.session[[g]] <- init.SMR.Dcov.Generalized.Interspersed(data[[g]],inits.use,M=M[g])
  }
  anyTelemetry <- FALSE
  ...
  K2D.sight <- array(NA,dim=c(N.session,max(J.sight),max(K.sight)))
  y.mark <- array(NA,dim=c(N.session,maxM,max(J.mark)))
  y.mID <- array(NA,dim=c(N.session,max(n.marked),max(J.sight),max(K.sight)))
  y.mnoID <- array(NA,dim=c(N.session,max(J.sight),max(K.sight)))
  y.um <- array(NA,dim=c(N.session,max(J.sight),max(K.sight)))
  y.unk <- array(NA,dim=c(N.session,max(J.sight),max(K.sight)))
  marked.status <- array(NA,dim=c(N.session,max(n.marked),max(K.sight)))
  ...
  for(g in 1:N.session){
    ...
    K2D.sight[g,1:J.sight[g],1:K.sight[g]] <- init.session[[g]]$K2D.sight
    y.mark[g,1:M[g],1:J.mark[g]] <- init.session[[g]]$y.mark
    y.mID[g,1:n.marked[g],1:J.sight[g],1:K.sight[g]] <- init.session[[g]]$y.mID
    y.mnoID[g,1:J.sight[g],1:K.sight[g]] <- init.session[[g]]$y.mnoID
    y.um[g,1:J.sight[g],1:K.sight[g]] <- init.session[[g]]$y.um
    y.unk[g,1:J.sight[g],1:K.sight[g]] <- init.session[[g]]$y.unk
    marked.status[g,1:n.marked[g],1:K.sight[g]] <- init.session[[g]]$marked.status
    xlim[g,] <- data[[g]]$xlim
    ylim[g,] <- data[[g]]$ylim
  ...
  return(list(y.mark=y.mark,y.mID=y.mID,y.mnoID=y.mnoID,y.um=y.um,y.unk=y.unk,marked.status=marked.status,
              s.init=s,z.init=z,K1D.mark=K1D.mark,K2D.sight=K2D.sight,J.mark=J.mark,J.sight=J.sight,K.sight=K.sight,
