NimModel <- nimbleCode({
  #baseline density for marked and unmarked individuals seperately
  D0.M ~ dunif(0,100) #uninformative, diffuse dnorm on log scale can cause neg bias
  D0.UM ~ dunif(0,100) #uninformative, diffuse dnorm on log scale can cause neg bias
  D0 <- D0.M + D0.UM
  ...
  lambda.N.M <- D.intercept.M*pi.denom #Expected marked N
  lambda.N.UM <- D.intercept.UM*pi.denom #Expected unmarked N
  ...
  for(i in (M1+1):M.both){
    ...
    dummy.data[i] ~ dCell(pi.cell[s.cell[i]])
    lam[i,1:J] <- GetDetectionRate(s = s[i,1:2], X = X[1:J,1:2], J=J,sigma=sigma, lam0=lam0, z=z[i])
  }#custom Metropolis-Hastings update for N.UM/z[(M1+1):M]
