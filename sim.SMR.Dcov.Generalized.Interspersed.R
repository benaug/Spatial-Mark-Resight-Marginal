...
sim.SMR.Dcov.Generalized.Interspersed <-
  function(D.beta0=NA,D.beta1=NA,D.cov=NA,InSS=NA,res=NA,
    ...
    y.um <- apply(y.unmarked,c(2,3),sum)
    if(!all(is.na(y.marked.noID))){
      y.mnoID <- apply(y.marked.noID,c(2,3),sum)
    }else{
      y.mnoID <- matrix(0,J,K)
    }
    if(!all(is.na(y.unk))){
      y.unk <- apply(y.unk,c(2,3),sum)
    }else{
      y.unk <- matrix(0,J,K)
    }
    ...
    out <- list(y.mark=y.mark,y.mID=y.marked,y.mnoID=y.mnoID,y.um=y.um,y.unk=y.unk,marked.status=marked.status, #observed data
