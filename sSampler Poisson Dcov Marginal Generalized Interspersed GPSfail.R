sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    y.mID.nodes <- model$expandNodeNames(paste("y.mID[",i,",1:",J.sight,",1:",K.sight,"]"))
    ...
    lam.mID.nodes <- model$expandNodeNames(paste("lam.mID[",i,",1:",J.sight,",1:",K.sight,"]"))
    lam.mnoID.nodes <- model$expandNodeNames(paste("lam.mnoID[1:",J.sight,",1:",K.sight,"]"))
    lam.um.nodes <- model$expandNodeNames(paste("lam.um[1:",J.sight,",1:",K.sight,"]"))
    lam.unk.nodes <- model$expandNodeNames(paste("lam.unk[1:",J.sight,",1:",K.sight,"]"))
    ...
    if(z==0){ #propose from prior
      ...
      if(inbox){
        ...
        if(i<=n.marked){ #if marked
          ...
          bigLam.marked1.initial <- model$bigLam.marked1
          bigLam.marked2.initial <- model$bigLam.marked2
          ...
          bigLam.marked1.proposed <- bigLam.marked1.initial
          bigLam.marked2.proposed <- bigLam.marked2.initial
          bigLam.unmarked.proposed <- bigLam.unmarked.initial
          for(k in 1:K.sight){
            bigLam.marked1.proposed[1:J.sight,k] <- bigLam.marked1.proposed[1:J.sight,k] - model$lam[i,1:J.sight]*model$mark.class1[i,k]
            bigLam.marked2.proposed[1:J.sight,k] <- bigLam.marked2.proposed[1:J.sight,k] - model$lam[i,1:J.sight]*model$mark.class2[i,k]
            bigLam.unmarked.proposed[1:J.sight,k] <- bigLam.unmarked.proposed[1:J.sight,k] - model$lam[i,1:J.sight]*model$mark.class0[i,k]
          }
          #make sure you didn't end up with any negative numbers due to machine precision
          for(j in 1:J.sight){
            for(k in 1:K.sight){
              if(bigLam.marked1.proposed[j,k] < 0){
                bigLam.marked1.proposed[j,k] <- 0
              }
              if(bigLam.marked2.proposed[j,k] < 0){
                bigLam.marked2.proposed[j,k] <- 0
              }
              if(bigLam.unmarked.proposed[j,k] < 0){
                bigLam.unmarked.proposed[j,k] <- 0
              }
            }
          }
          ...
          model$bigLam.marked1[1:J.sight,1:K.sight] <<- bigLam.marked1.proposed[1:J.sight,1:K.sight]
          model$bigLam.marked2[1:J.sight,1:K.sight] <<- bigLam.marked2.proposed[1:J.sight,1:K.sight]
          model$bigLam.unmarked[1:J.sight,1:K.sight] <<- bigLam.unmarked.proposed[1:J.sight,1:K.sight]
          model$calculate(lam.mID.nodes) #update after lam
          ...
          bigLam.unmarked.proposed <- bigLam.unmarked.initial
          for(k in 1:K.sight){
            bigLam.unmarked.proposed[1:J.sight,k] <- bigLam.unmarked.proposed[1:J.sight,k] - model$lam[i,1:J.sight]
          }
          #make sure you didn't end up with any negative numbers due to machine precision
          for(j in 1:J.sight){
            for(k in 1:K.sight){
