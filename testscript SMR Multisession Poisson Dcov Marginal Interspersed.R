#multisession version of Poisson Dcov Marginal Interspersed

library(nimble)
source("sim.SMR.multisession.Dcov.Interspersed.R")
source("sim.SMR.Dcov.Interspersed.R") #used by multisession simulator
source("NimbleModel SMR Multisession Poisson Dcov Marginal Interspersed.R")
source("NimbleFunctions SMR Multisession Poisson Dcov Marginal Interspersed.R")
source("init.SMR.multisession.Dcov.Interspersed.R")
source("init.SMR.Dcov.Interspersed.R") #used by multisession initializer
source("sSampler Multisession Poisson Dcov Marginal Interspersed.R")
source("mask.check.R")
...
#to test interspersed marking and resighting, we will consider that some individuals were marked halfway through
#the survey.
p.half <- 0.5 #probability a marked individual is not marked until halfway through the study
...
set.seed(1343532) #change seed for new data set
data <- sim.SMR.multisession.Dcov.Interspersed(N.session=N.session,n.marked=n.marked,
...
str(data[[g]]$marked.status) #history of which occasions marked individuals are marked
...
inits <- list(lam0=rep(1,N.session),sigma=rep(1,N.session),theta.marked=theta.marked.init,theta.unmarked=theta.unmarked.init) #initializing with 1 parameter per session, just set all to same value
...
# constants <- list(N.session=N.session,n.marked=n.marked,M=M,J=nimbuild$J,K2D=nimbuild$K2D,K=nimbuild$K,
...
constants <- list(N.session=N.session,n.marked=n.marked,M=M,J=J,K2D=nimbuild$K2D,K=nimbuild$K,
                  xlim=nimbuild$xlim,ylim=nimbuild$ylim,D.cov=nimbuild$D.cov,res=nimbuild$res,
                  cellArea=nimbuild$cellArea,n.cells=nimbuild$n.cells,
                  #telemetry stuff
...
for(g in 1:N.session){
  ...
  y.um.nodes <- Rmodel$expandNodeNames(paste("y.um[",g,",1:",J[g],",1:",K[g],"]"))
  y.unk.nodes <- Rmodel$expandNodeNames(paste("y.unk[",g,",1:",J[g],",1:",K[g],"]"))
  bigLam.unmarked.nodes <- Rmodel$expandNodeNames(paste("bigLam.unmarked[",g,",1:",J[g],"]")) #only need this in calcNodes
  lam.um.nodes <- Rmodel$expandNodeNames(paste("lam.um[",g,",1:",J[g],",1:",K[g],"]"))
  lam.unk.nodes <- Rmodel$expandNodeNames(paste("lam.unk[",g,",1:",J[g],",1:",K[g],"]"))
  ...
  conf$addSampler(target = paste("N"),
                  type = 'zSampler',control = list(g=g,z.ups=z.ups[g],
                                                   J=J[g],K=K[g],n.marked=n.marked[g],M=M[g],
                                                   lam.nodes=lam.nodes,
...
#                     type = 'sSampler',control=list(i=i,g=g,J=nimbuild$J[g],K=nimbuild$K[g],n.cells=nimbuild$n.cells[g],
...
for(g in 1:N.session){
  for(i in 1:M[g]){
    if(i %in% nimbuild$tel.inds[g,]){ #inds with telemetry
      conf$addSampler(target = paste("s[",g,",",i,", 1:2]", sep=""),
                      type = 'sSampler',control=list(i=i,g=g,J=J[g],K=nimbuild$K[g],res=nimbuild$res[g],n.cells=nimbuild$n.cells[g],
                                                     n.cells.x=nimbuild$n.cells.x[g],n.cells.y=nimbuild$n.cells.y[g],
                                                     xlim=nimbuild$xlim[g,],ylim=nimbuild$ylim[g,],res=nimbuild$res[g],
