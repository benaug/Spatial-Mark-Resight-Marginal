...
#It also considers that marks that lose their ability to provide individual ID information, but
#remain on the animal. This is designed for the situation where individual ID is determined
#by GPS telemetry locations. If the GPS collar dies but remains on the animal, you can tell
#it is marked but you can't determine an individual ID (typically). In these models that
#allow all sample types, failure to split the probabilities between working and dead GPS-marked
#individuals will introduce bias to some extent. Other multiple mark type scenarios can be
#handled with the model code, but data simulator is for the GPS scenario described above.

#Another assumption of the data simulator is that if you recapture a dead GPS individual, 
#you always replace it with a new one. Model file allows you to do whatever you want,
#put on new collar, leave dead one on, or remove collar.
...
library(nimble)
source("sim.SMR.Dcov.Generalized.Interspersed.GPSfail.R")
source("NimbleModel SMR Poisson Dcov DA2 Marginal Generalized Interspersed GPSfail.R")
source("NimbleFunctions SMR Poisson Dcov DA2 Marginal Generalized Interspersed GPSfail.R")
source("init.SMR.Dcov.Generalized.Interspersed.GPSfail.R")
source("sSampler Poisson Dcov Marginal Generalized Interspersed GPSfail.R")
source("mask.check.R")
...
#P(ID, Marked no ID, unk status). must sum to 1
theta.marked1 <- c(0.75,0.15,0.1) #working GPS
theta.marked2 <- c(0,0.9,0.1) #dead GPS
...
p.fail <- 0.2 #per occasion probability working GPS collar fails and can no longer provide certain ID
...
data <- sim.SMR.Dcov.Generalized.Interspersed.GPSfail(D.beta0=D.beta0,D.beta1=D.beta1,res=res,
                D.cov=D.cov,InSS=InSS,
                theta.marked1=theta.marked1,theta.marked2=theta.marked2,
                theta.unmarked=theta.unmarked,
                lam0=lam0,p0=p0,sigma=sigma,
                K.mark=K.mark,K.sight=K.sight,K.order=K.order,p.fail=p.fail,
...
str(data$marked.status) #history of which sighting occasions marked individuals are marked *and by which mark type*
...
#each must sum to 1
theta.marked1.init <- c(0.5,0.25,0.25)
theta.marked2.init <- c(0,0.5,0.5) #make sure any impossible events are 0
theta.unmarked.init <- c(0,0.75,0.25) #first cell for theta.unmarked.init should be 0
inits <- list(p0=0.5,lam0=1,sigma=1,theta.marked1=theta.marked1.init,theta.marked2=theta.marked2.init,
              theta.unmarked=theta.unmarked.init)

#This function structures the simulated data to fit the model in Nimble (some more restructing below)
#Also checks some inits
nimbuild <- init.SMR.Dcov.Generalized.Interspersed.GPSfail(data,inits,M=M)
...
mark.class0 <- matrix(0,M,K.sight)
mark.class1 <- matrix(0,M,K.sight)
mark.class2 <- matrix(0,M,K.sight)
mark.class0[1:n.marked,] <- 1*(data$marked.status==0)
mark.class1[1:n.marked,] <- 1*(data$marked.status==1)
mark.class2[1:n.marked,] <- 1*(data$marked.status==2)
mark.class0[(n.marked+1):M,] <- 1
...
parameters <- c('D0','lambda.N','p0','lam0','sigma','theta.marked1','theta.marked2','theta.unmarked',
                'N','D.beta1')
...
config.nodes <- c('p0','lam0','sigma','theta.marked1','theta.marked2[2:3]','theta.unmarked[2:3]')
# config.nodes <- c()
