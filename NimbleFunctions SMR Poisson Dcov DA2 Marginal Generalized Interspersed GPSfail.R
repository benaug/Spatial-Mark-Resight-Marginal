      ...
      if(updown==0){#subtract
        ...
        if(n.z.on>0){ #skip if no unmarked z's to turn off, otherwise nimble will crash
          ...
          if(!reject){
            ...
            bigLam.unmarked.proposed <- bigLam.unmarked.initial #subtract these out before calculate
            for(k in 1:K.sight){
              bigLam.unmarked.proposed[1:J.sight,k] <- bigLam.unmarked.proposed[1:J.sight,k] - model$lam[pick,1:J.sight]
            }
            ...
            if(accept) {
              ...
              mvSaved["bigLam.unmarked",1][1:J.sight,1:K.sight] <<- model[["bigLam.unmarked"]][1:J.sight,1:K.sight]
              ...
              model[["bigLam.unmarked"]][1:J.sight,1:K.sight] <<- mvSaved["bigLam.unmarked",1][1:J.sight,1:K.sight]
        ...
        if(model$N[1] < M){ #cannot update if z maxed out. Need to raise M
          ...
          model$calculate(pd.nodes[pick])
          model$calculate(lam.nodes[pick])
          bigLam.unmarked.proposed <- bigLam.unmarked.initial #add these in after calculate
          for(k in 1:K.sight){
            bigLam.unmarked.proposed[1:J.sight,k] <- bigLam.unmarked.proposed[1:J.sight,k] + model$lam[pick,1:J.sight]
          }
          model$bigLam.unmarked[1:J.sight,1:K.sight] <<- bigLam.unmarked.proposed[1:J.sight,1:K.sight]
